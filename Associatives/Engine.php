    ...
    class Engine extends \Aomebo\Singleton
    {
        ...
        public function __construct()
        {
            ...
            if (!self::_isConstructed()) {
                ...
                $this->_scanDependencies();
                $this->_scanModules();
                $this->_scanAssociatives();
        ...
        public function getAssociativeData()
        {
            ...
            if (!self::$_supressOutput) {
                $tmp .= $this->_getDependencyData();
                $tmp .= $this->_getAssociativeData();
            }
        ...
        /**
         * This function scans filesystem for
         * all availible modules.
         *
         * @internal
         * @throws \Exception
         */
        private function _scanModules()
        {
            ...
            foreach ($roots as $root)
            {
                ...
                foreach ($dirs as $dir)
                {
                    ...
                    $moduleCsName = $dir;
                    $moduleCisName = strtolower($dir);

                    if ($dir != '.'
                        && $dir != '..'
                        && is_dir($path)
                        && !isset(self::$_associatives[$moduleCisName])
                    ) {

                        $namespaceName = basename(dirname($path));
                        $namespaceClassName = substr($namespaceName, 0, -1);

                        $file = $path . DIRECTORY_SEPARATOR
                            . $namespaceClassName . _PHP_EX_;
                        $alternateFile = $path . DIRECTORY_SEPARATOR
                            . $dir . _PHP_EX_;

                        if (file_exists($file)
                            || file_exists($alternateFile)
                        ) {

                            self::$_modules[$moduleCisName] =
                                array(
                                    'cs_name' => $moduleCsName,
                                    'cis_name' => $moduleCisName,
                                );
         ...
         * @throws \Exception
         */
        private function _scanAssociatives()
        {

            $session = \Aomebo\Session\Handler::getInstance();
            $sessionBlock = $session->getSessionBlock();
            $associativesDir = \Aomebo\Configuration::getSetting('paths,associatives dir');
            $dispatcher = \Aomebo\Dispatcher\System::getInstance();
            $requestType = ($dispatcher::isAjaxRequest() ? self::REQUEST_TYPE_AJAX : self::REQUEST_TYPE_DEFAULT);
            ...
            foreach (self::$_modules as & $module)
            {
                ...
                if (class_exists($moduleName, false)) {
                    ...
                    if ($object->isAssociatable()) {

                        /** @var \Aomebo\Runtime\Associatable $object */
                        $options = $object->getAssociatives();
                ...
                foreach ($roots as $root)
                {
                    ...
                    $dirs = array(
                        $root . DIRECTORY_SEPARATOR
                            . $module['cs_name'] . DIRECTORY_SEPARATOR
                            . $associativesDir,
                        $root . $associativesDir . DIRECTORY_SEPARATOR
                            . $module['cs_name']);
                                ...
                                foreach ($files as $file)
                                {
                                    ...
                                    if ($file != '.'
                                        ...
                                        if ($fileSessionApproved
                                            ...
                                            if (!isset(self::$_associatives[$module['cis_name']]))
                                            {
                                                self::$_associatives[$module['cis_name']] =
                                                    array(
                                                        'cis_name' => $module['cis_name'],
                                                        'cs_name' => $module['cs_name'],
                                            ...
                                            $associative = & self::$_associatives[$module['cis_name']];
         ...
         * @throws \Exception
         */
        private function _scanDependencies()
        {
            $session = \Aomebo\Session\Handler::getInstance();
            $sessionBlock = $session->getSessionBlock();
            $roots = array(
         ...
         * @return string
         */
        private function _getDependencyData()
        {
         ...
         * @return string
         */
        private function _getAssociativeData()
        {
            $dispatcher = \Aomebo\Dispatcher\System::getInstance();
            $parser = \Aomebo\Associatives\Parser::getInstance();
            $return = '';
